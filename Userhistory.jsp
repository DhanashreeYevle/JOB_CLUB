...
<title>History of Job Applied</title>
<style type="text/css">
p{
	color:darkred;
}
...
<center><h1 style="color:blue">History</h1><sql:setDataSource var="db" driver="com.mysql.jdbc.Driver" url="jdbc:mysql://localhost:3306/register" user="root" password="root"/>
<sql:query dataSource="${db}" var="rs">
Select * from apply where u_id = '${mobile}'
</sql:query>
<table>
<c:forEach var="table" items="${rs.rows}">
		<table width="40%"><tr><td>Applied On:</td><td><c:out value="${table.time}"></c:out></td></tr>
		...
		<tr><td>PIN Code:</td><td><c:out value="${table.pin}"></c:out></td></tr><tr>
		<td><a href ="uhupdate.jsp?w_id=<c:out value='${table.w_id}'/>">
		<button class="update">Update</button></td></a>
		<td><a href ="delete.jsp?w_id=<c:out value='${table.w_id}'/>">
		<button class="delete">Delete</button></td></a></td>
	</tr></table><br>
</c:forEach></table>
</center>
</body>
